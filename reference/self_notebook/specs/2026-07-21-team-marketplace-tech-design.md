...
### 决策 1 — blob 托管与安装路径 ★需拍板

`ee1db871` 原设计 blob 挂 narra.nexus 静态,`bundle_url` 指向外部,安装走
`/import/from-url`(后端自己 HTTP 拉取 + host 白名单)。你的要求是 **blob 上
S3(暂本地)、与 skill 分开**。因此我们自托管 blob,安装有两条路:

- **方案 A(最大复用)**:catalog 存 `store_key`,后端加
  `GET /api/marketplace/teams/templates/{id}/download` 服务 `.nxbundle`;
  前端安装 = 现有 `importBundleFromUrl(downloadUrl, sha256)`(已存在)。
  代价:后端自己 HTTP 拉自己的 download URL,需要把自身 host 加进
  `BUNDLE_FETCH_ALLOWED_HOSTS`(bundle.py 已为「未来 local-marketplace」预留
  了 localhost 白名单钩子)。
- **方案 B(更干净,推荐)**:新增
  `POST /api/marketplace/teams/templates/{id}/install-preflight`——后端从
  artifact store 取 `.nxbundle` → 校验 sha256 → **直接**调 `importer.preflight()`
  → 返回与 `/import/preflight` 同 shape 的 `{preflight_token, manifest,
  clashes...}`;前端复用现有 review UI,confirm 走现成的
  `POST /api/bundle/import/confirm`。**不自拉 HTTP、不依赖 host 白名单**,
  只跳过「HTTP 自取」这一步,importer 的真正工作全复用。

**AI 建议:方案 B**。理由:blob 已在后端手里,自己 HTTP 请求自己是绕路;
B 的新增代码就一个端点(取文件 → 验 hash → preflight),且不碰 bundle.py 的
白名单逻辑。
...
| `POST /templates/{id}/install-preflight` | store 取 bundle → 验 sha256 → importer.preflight → 返回 preflight_token + manifest + clashes(与 /import/preflight 同 shape) | 登录 |
