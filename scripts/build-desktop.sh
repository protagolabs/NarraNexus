...
echo ""
echo "--- Step 3.6: Installing bundled CLIs (claude-code + lark-cli + officecli) ---"
...
for bin in claude lark-cli officecli; do
...
# Same treatment for officecli. @officecli/officecli's bin is a Node launcher
# (officecli.js) that does `require('./lib/install-binary')` and reads the
# native binary from `<pkg>/vendor/` — both resolved via `__dirname`. Tauri's
# symlink-flattening turns `.bin/officecli` into a flat copy whose `__dirname`
# is `.bin/`, so `require('./lib/install-binary')` → MODULE_NOT_FOUND (identical
# failure mode to lark-cli). The bash shim runs the REAL officecli.js with the
# bundled node, restoring correct `__dirname` (→ vendor/ + lib/ resolve). The
# native binary was fetched into `@officecli/officecli/vendor/` by the npm ci
# postinstall above, so it ships inside the .app — no first-run network needed.
echo "  Writing bash shim for officecli (works around Tauri bundler symlink flattening)..."
cat > "$NODE_DIR/node_modules/.bin/officecli" <<'OFFICE_SHIM'
#!/usr/bin/env bash
# Auto-generated by scripts/build-desktop.sh. Do NOT edit by hand here —
# edit the shim template in build-desktop.sh and rebuild.
#
# $(dirname "$0") resolves to node_modules/.bin/ at runtime regardless of
# whether this file was a symlink or a flat copy. Run the real launcher with
# the bundled node so officecli.js's __dirname points at the real package dir.
exec "$(dirname "$0")/../../bin/node" "$(dirname "$0")/../@officecli/officecli/officecli.js" "$@"
OFFICE_SHIM
chmod +x "$NODE_DIR/node_modules/.bin/officecli"
...
# officecli's shim needs node on PATH (it execs the bundled node). Point PATH at
# the bundled node bin for this smoke test.
_office_ver=$(PATH="$NODE_DIR/bin:$PATH" "$NODE_DIR/node_modules/.bin/officecli" --version 2>&1 | head -1 || true)
if echo "$_office_ver" | grep -qi "cannot find\|module_not_found\|Error:\|failed to launch"; then
    echo "  officecli: ❌ launcher/binary not working — output:"
    echo "$_office_ver" | sed 's/^/    /'
    echo "  Build aborted: officecli would not work in the shipped dmg."
    exit 1
fi
echo "  officecli: $_office_ver"
