...
for arg in "$@"; do
    case "$arg" in
        --skip-signing)
            ...
            ;;
        --help|-h)
            echo "Usage: bash scripts/release/build-desktop.sh [OPTIONS]"
            echo ""
            echo "Options:"
...
verify_sha256() {
    ...
    if ! printf '%s  %s\n' "$expected" "$file" | shasum -a 256 -c --status -; then
        ...
        echo "    Bump the expected value in scripts/release/build-desktop.sh only after verifying the"
...
#!/usr/bin/env bash
# Auto-generated by scripts/release/build-desktop.sh — do not edit here.
...
#   scripts/release/build-desktop.sh (here)    the macOS app bundle
...
#
# Escape hatch: `SKIP_LARK_SKILLS=1 bash scripts/release/build-desktop.sh` skips
...
if [ -n "${SKIP_LARK_SKILLS:-}" ]; then
    ...
    if [ -d "$SKILL_STAGE/.agents/skills" ]; then
        ...
        echo "        SKIP_LARK_SKILLS=1 bash scripts/release/build-desktop.sh"
        rm -rf "$SKILL_STAGE"
    fi
fi
...
#!/usr/bin/env bash
# Auto-generated by scripts/release/build-desktop.sh. Do NOT edit by hand here —
