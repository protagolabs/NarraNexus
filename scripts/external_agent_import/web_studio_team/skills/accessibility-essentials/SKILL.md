...
description: How to QA the site for accessibility — WCAG 2.1 AA basics, semantic HTML, color contrast, keyboard navigation, alt-text. Use whenever you're reviewing the built site, before declaring it ready. Pair with the playwright-mcp skill to actually inspect the rendered output.
---

# Accessibility Review — essentials

Target: **WCAG 2.1 Level AA** baseline. We're not chasing AAA in a 1-hour POC, but AA is non-negotiable for an branded site.
...
|---|---|---|---|
| 1 | **Page title** | `<title>` is specific, mentions project + date | `browser_snapshot` → title field |
...
The QA role also verifies copy matches the project-brief-template:
- Dates say "<dates from project_brief.md>" ✓
- Free is mentioned in hero or above the fold ✓
- The 4 partner institutions are listed correctly ✓ (<project entities>, Science Museum, V&A, Royal Albert Hall)
