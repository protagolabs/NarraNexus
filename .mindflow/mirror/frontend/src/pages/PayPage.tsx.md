...
| 状态 | 行为 |
|---|---|
| 已登录 × free | getSubscription 确认非 ACTIVE → subscribe → 直跳 Stripe |
| 未登录 | ProtectedRoute → `/login?next=%2Fpay`,登录/注册后回来续跳(next 链路三条登录路径都支持,LoginPage 已有) |
| 已订阅(ACTIVE) | `/app/settings?tab=account`(去管理,不重复铸 session) |
| 非 Power 账号(无 netmindToken) | 同上——没 token 没法付,账户页会解释账号状态 |
| subscribe 失败 | 行内错误 + 重试 + 账户页链接,绝不静默死胡同 |

## 设计决策

- **同 tab `location.assign`,不用 platform.openExternal**:用户从官网点进来,
  这个 tab 的唯一使命就是结账;Stripe 付完的 return URL 回
  `/app/settings?tab=account`(billing.py::_return_urls 已有),闭环。
- **inFlight ref 防重**:StrictMode 开发态 effect 双触发,不能铸两个
  checkout session;重试路径同样过这个闸。
- **先查订阅再 subscribe**:防止已订阅用户重复付款——顺序是刻意的。
...
- 测试:`pages/__tests__/PayPage.test.tsx`(6 个分支)。
