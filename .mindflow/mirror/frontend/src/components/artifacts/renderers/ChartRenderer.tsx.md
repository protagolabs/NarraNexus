...
延迟 init 的 `init()` 函数体包 try/catch:它从 ResizeObserver 回调(另一调用栈)触发,抛错逃逸外层 catch → 延迟 init 路径失败会永久空白无红字(review ②);失败 setError + observer.disconnect 止重抛。register 改数组语义(见 [[artifactStore.ts]])。
