...
Rows come from the `ui.channels` registry (`sortedChannels(useRegistryEntries(CHANNELS))`) — the six builtins register in `registerBuiltinChannels.ts` with their plugin id as owner, a channel plugin registers its own — instead of the hard-coded `IM_CHANNELS` list; probing, expand/collapse and the counter are unchanged.

## 2026-09-07 · `ChannelConfigProps` no longer re-exported from here (I-8 fix)

Every config component now imports `ChannelConfigProps` straight from
`@/platform/registries` (the previous re-export through this file created a
`registerBuiltinChannels → *Config → IMChannelsSection` circular import that
`.dependency-cruiser.cjs`'s `no-circular` rule flagged as a `warn`). The
doc-comment claim "Adding a future channel is just one entry in
`IM_CHANNELS`" was already stale before this batch (the array is gone —
rows come from the `ui.channels` registry) and has been corrected in the
file header.
