...
迁移核心逻辑（`classify_identity_columns`、`execute_migration`、`verify_migration`、`build_report`、`IdentityColumns`）已移至 `backend/integrations/netmind/identity_migration.py`（platform service 层，铁律 3），由 `backend/routes/admin_migration.py` 和本脚本共享（铁律 8，单一真相源）。
