---
code_file: src/xyz_agent_context/services/feedback_client.py
...
- **隐私契约在此层强制**：agent_id/user_id 一律 sha256[:16] 哈希后出境；
  summary 截 500 字符；空摘要不发。调用方（MCP 工具/backend 路由）不需要
  各自记得脱敏。
