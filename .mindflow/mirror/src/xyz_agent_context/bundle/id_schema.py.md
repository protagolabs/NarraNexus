...
`art_[0-9a-f]{8,16}`。注意：artifact 注册实现(`xyz_agent_context/artifact`)现存代码用 `secrets.token_hex(4)` = 8-hex，bundle 端 `gen_new_id("artifact")` 用 `secrets.token_hex(6)` = 12-hex；范围 8..16 同时覆盖两套。
