...
expressive/disallowed 三者合成一个 `ToolSurface`。`ModuleConfig.enabled` 进 `CapabilityMeta.requires["enabled"]`，
不丢字段（ModuleConfig 五字段全部有落点）。`tests/module/test_capability_adapter.py` 用真实
