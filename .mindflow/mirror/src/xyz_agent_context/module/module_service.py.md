...
**`module_registry` 懒加载**：在 `__init__` 里通过 `from xyz_agent_context.module import module_registry` 动态导入，而非模块顶部导入。这是为了避免循环引用：`module_service.py` 被 `__init__.py` 导出，而 `__init__.py` 又导入了所有具体 Module 类，如果在顶部静态导入 `module_registry` 就会形成循环。
