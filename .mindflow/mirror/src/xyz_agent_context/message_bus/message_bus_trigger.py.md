...
**遗留一个没人拥有的量**：公告栏有硬顶（条目 2000 字 + 总结 800 字）且空态零字符；
工作板在渲染处没有上限，且空态仍输出 `- (no open work items)`。两块相加没有总预算，
而 20 条 scrollback 还要挤在同一个 prompt 里。这不是本次 merge 该单方面定的事，已在 PR 上单独提出。
