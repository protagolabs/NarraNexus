...
**Cross-process-safe transactions (token + write-gate + watchdog).** The proxy holds ONE shared SQLite connection for all client processes, so a naive per-client transaction over that connection is unsafe in two ways: a non-owner process's write executed while a transaction is open gets folded into it and committed/rolled-back with it (silent data loss, "P1"), and a client that dies mid-transaction leaves the connection stuck in a transaction forever, disabling auto-commit process-wide ("P4"). The fix: `/transaction/begin` claims a single global slot and issues a `txn_id`; every write endpoint (`/execute_write`, `/insert`, `/update`, `/delete`, `/upsert`) passes through `_await_txn_turn`, which admits the holder's own writes (matching `txn_id`) immediately and blocks all others on the `_txn_done` event until the transaction ends. `/transaction/commit` and `/transaction/rollback` validate the token and release the slot in a `finally` (so an error can never leave the slot stuck). A background watchdog (`_transaction_watchdog` → `_reap_expired_txn`) force-rolls-back any transaction still open past `_TXN_TIMEOUT` and logs a warning — a diagnostic auto-recovery, not a policy limit. Reads (`/get`, `/get_one`, `/get_by_ids`) are intentionally NOT gated: P1 is a write-folding problem, and gating reads would needlessly serialize them behind writes (dirty reads on the shared connection are a pre-existing, low-risk concern). The transaction primitives (`_txn_done`, `_state_lock`) are created in the lifespan startup via `_reset_txn_state()` so they bind to the proxy's own event loop.
