...
- `backend/routes/agents/attachments.py::upload_attachment` —
  the only production caller. Imports `TranscriptionService.instance()`
  for both `is_available(user_id)` and `transcribe(...)`.
- `backend/routes/transcription/routes.py::availability` — frontend mic-button
  pre-flight; calls `service.availability_reason(user_id)`.
- `backend/routes/transcription/public.py::fetch_audio` — NetMind's
