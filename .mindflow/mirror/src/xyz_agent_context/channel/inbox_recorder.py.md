...
调用点接线:`channel_trigger_base.py`(2 处生产)+ `lark_trigger.py:_process_message`(1 处生产)传 `chat_id=message.chat_id, chat_type=message.chat_type`;`_write_to_inbox`(lark 测试 shim)传 `chat_id + chat_type=ChatType.PRIVATE`(保持生产形状,**会**记 reach——意味着经该 shim 的既有 lark 测试会触到 seam 自己的句柄)。**⚠️ Lark 的 `chat_type` 判据(正向白名单 `PRIVATE if raw=="p2p" else GROUP`)与它的守卫见 [[lark_trigger.py]] 同日条目——单一来源,不在此复述以免漂移。** 本文件守卫 `test_inbox_reach_recording.py`(真 db:1:1 记下→读回、group/topic 不记、无 chat_id 不记、create 带名、已存在无名被填、非空名不覆盖、in-band 失败打 warning 且不丢 inbox 行——删 try/except 或删 success 检查即变红)+ `test_mock_channel_trigger_integration.py`(spy 锁调用点传下的是消息真实 `chat_type`:fake 产 GROUP→到达 GROUP 且不记 reach、产 PRIVATE→PRIVATE;删 kwarg 或换硬编码常量都变红)。
