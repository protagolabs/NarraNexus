...
**Fail-closed on unexpected errors**：除 uniqueness 外的任何 insert 异常都
log + return False（当作已见）。理由：这里宁可偶尔丢一条消息，也不能让重复执行
继续污染用户体验。trigger 层还有 "no id → process defensively" 的 fail-open
fallback 兜着，系统整体不会静默死锁。
