---
code_file: src/narranexus/platform/repository/instance_awareness_repository.py
...
- The `InstanceAwareness` dataclass defined in this file is not in `schema/` — it lives in the repository file itself. This is an exception to the pattern of keeping all data models in `schema/`. If you need to import `InstanceAwareness`, import it from `narranexus.platform.repository.instance_awareness_repository`.
