---
code_file: src/narranexus/platform/module_system/module_service.py
...
- **依赖谁**：`ModuleLoader`（`_module_impl/loader.py`）做实际工作；`module_registry`（`__init__.py`）提供类注册表；`DatabaseClient` 透传给 `ModuleLoader`
...
**`module_registry` 懒加载**：在 `__init__` 里通过 `from narranexus.platform.module_system import module_registry` 动态导入，而非模块顶部导入。这是为了避免循环引用：`module_service.py` 被 `__init__.py` 导出，而 `__init__.py` 又导入了所有具体 Module 类，如果在顶部静态导入 `module_registry` 就会形成循环。
...
- 以为 `get_all_module_names()` 返回的是"当前已加载的模块"——它返回的是 `module_registry` 里注册的所有可用模块名，与当次调用无关。
