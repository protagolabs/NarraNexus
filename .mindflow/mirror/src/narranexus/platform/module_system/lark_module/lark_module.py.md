---
code_file: src/narranexus/platform/module_system/lark_module/lark_module.py
stub: false
...
`command` 不过 shell**，同时又鼓励把整篇报告内联传入。正文一到 10KB+，模型的本能
就是 `$(cat file)` —— 而那会被原样写进文档且返回 success（见
[[_lark_command_security]]）。新增的 `_NO_SHELL_GUIDE` 正面讲清三件事不生效
（`$(...)` / `-` stdin / heredoc）、且**不会报错**，并给出唯一可用的
`--content @relative/path.md`（相对路径，绝对路径被 CLI 拒）。铁律 #1 同步改写。
...
`LarkModule` now inherits `narranexus.platform.channel.ChannelModuleBase`
...
`.mindflow/mirror/src/narranexus/platform/module_system/chat_module/chat_module.py.md`
2026-04-23 那一段）。
