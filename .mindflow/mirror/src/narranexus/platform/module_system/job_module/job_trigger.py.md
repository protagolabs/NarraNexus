---
code_file: src/narranexus/platform/module_system/job_module/job_trigger.py
...
- **被谁用**：`run.sh` / `Makefile` 通过 `python -m narranexus.platform.module_system.job_module.job_trigger` 直接启动；Tauri desktop 通过 sidecar 启动
