---
code_file: src/narranexus/platform/module_system/telegram_module/_telegram_mcp_tools.py
stub: false
...
`_get_credential` 不再 `get_mcp_db_client()`+manager 直连库，改 `get_channel_credential_store().get_credential("telegram", …)` → `_cred_from_raw` 重建 dataclass，工具里 cred.bot_token 用法零变化；本地 DirectStore、云端 HttpStore 打 owner-gated 端点。**写留尾**：bind/status/unbind 经 `_get_manager`→`ChannelDirectStore().get_manager("telegram")`（本地专用、无 HttpStore 对应），故云端写工具仍需本地 db——写路径迁移前 mcp 摘不掉 DB_PASSWORD（见 [[channel_store]] 已知缺口）。


## 2026-07-24 — setup residency (B++): zero-arg `tg_bind` returns the guide

`tg_bind` called with empty credential args now returns `{"success": True,
"setup_guide": _NO_BOT_INSTRUCTION}` instead of a "required" error — the full
walkthrough left the system prompt ([[telegram_module]] unbound one-liner) and
