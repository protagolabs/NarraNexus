...
`set_enabled` 写入前对 reason 做一次 `str()`（PR #388 round-2 M6：前端/插件消费方据此可依赖字段恒为字符串，
不必各自 `typeof` 守卫）。`set_enabled` 捕获 `patch` 的版本竞争 `RuntimeError` → `logger.error` + 返回 False，保持"永不抛、False =
