---
code_dir: src/narranexus/platform/context_runtime/
...
- **`module/`** — `HookManager` is used to fire `gather` on every active module instance, allowing modules to inject data (most importantly `ctx_data.chat_history` from `ChatModule`) before the prompt is assembled. Module MCP URLs are also collected here.
