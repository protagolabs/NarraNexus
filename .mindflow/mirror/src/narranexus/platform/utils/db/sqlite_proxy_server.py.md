---
code_file: src/narranexus/platform/utils/db/sqlite_proxy_server.py
stub: false
---
...
**Cross-process-safe transactions (token + write-gate + watchdog).** The proxy holds ONE shared SQLite connection for all client processes, so a naive per-client transaction over that connection is unsafe in two ways: a non-owner process's write executed while a transaction is open gets folded into it and committed/rolled-back with it (silent data loss, "P1"), and a client that dies mid-transaction leaves the connection stuck in a transaction forever, disabling auto-commit process-wide ("P4"). The fix: `/transaction/begin` claims a single global slot and issues a `txn_id`; every write path passes through `_await_txn_turn`, which admits the holder's own writes (matching `txn_id`) immediately and blocks all others on the `_txn_done` event until the transaction ends. The gated endpoints are `/execute_write`, `/insert`, `/update`, `/delete`, `/upsert`, **and `/execute`** — the last one because raw SQL through `/execute` may be a write (a caller issuing an `UPDATE`/`DELETE` with `fetch=True` routes here, e.g. `ModulePoller` in a separate process); it cannot be cheaply proven read-only, so it is gated too, at the cost of serializing raw SELECTs during a (short) transaction. Only the typed reads (`/get`, `/get_one`, `/get_by_ids`) are ungated. `/transaction/commit` and `/transaction/rollback` validate the token and release the slot in a `finally` (so an error can never leave the slot stuck). A background watchdog (`_transaction_watchdog` → `_reap_expired_txn`) force-rolls-back any transaction still open past `_TXN_TIMEOUT` and logs a warning — a diagnostic auto-recovery, not a policy limit. `_TXN_TIMEOUT` (default 10s) is kept well below the client httpx timeout (30s) and is env-overridable (`SQLITE_PROXY_TXN_TIMEOUT`, `SQLITE_PROXY_TXN_WATCH_INTERVAL`) so an abandoned transaction is reaped before a write blocked behind it hits its own `ReadTimeout`. The transaction primitives (`_txn_done`, `_state_lock`) are created in lifespan startup via `_reset_txn_state()` so they bind to the proxy's own event loop.

Trade-off worth naming: `_await_txn_turn` blocks inside the HTTP handler, holding that client connection for the wait. With the client pool capped at 20 connections, a burst of >20 writes blocked behind one transaction could stall that process's reads too — bounded by the short `_TXN_TIMEOUT` and the fact that real transactions (`wipe_service`) are sub-second. A future improvement is a fast "retry-later" response instead of blocking, once client-side retry (P2) lands.
...
## 2026-09-04 · `main()` (batch 6a)

The uvicorn launch is `main() -> int`; the `-m` name compose/Tauri used stays runnable through the `xyz_agent_context` shim; the uvicorn app string is `narranexus.platform.utils.db.sqlite_proxy_server:app`.
