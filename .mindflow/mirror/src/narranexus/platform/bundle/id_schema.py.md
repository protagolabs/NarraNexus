---
code_file: src/narranexus/platform/bundle/id_schema.py
...
`art_[0-9a-f]{8,16}`。注意：artifact 注册实现(`narranexus/platform/artifact`)现存代码用 `secrets.token_hex(4)` = 8-hex，bundle 端 `gen_new_id("artifact")` 用 `secrets.token_hex(6)` = 12-hex；范围 8..16 同时覆盖两套。
