---
code_file: src/narranexus/platform/narrative/exporters.py
...
**依赖谁**：依赖 `narranexus.platform.settings.settings` 获取文件存储路径（`narrative_markdown_path` 和 `trajectory_path`），路径可通过环境变量覆盖。`TrajectoryRecorder` 还依赖 `ExecutionState` 和 `ModuleInstance` 类型（通过 TYPE_CHECKING 懒导入，避免循环依赖）。
