---
code_file: src/narranexus/platform/agent_framework/adapters/_tool_policy_guard.py
...
- **被谁装**：`agent_framework/adapters/claude/sdk.py` 在构造 `ClaudeAgentOptions` 时通过 `build_tool_policy_guard(workspace=..., supports_server_tools=..., mode=...)` 创建 hook，注册到 `HookMatcher(matcher="Read|Glob|Grep|WebSearch|Bash", hooks=[policy_guard])`
