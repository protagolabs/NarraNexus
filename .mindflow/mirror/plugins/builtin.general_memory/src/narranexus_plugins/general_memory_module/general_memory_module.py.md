---
code_file: plugins/builtin.general_memory/src/narranexus_plugins/general_memory_module/general_memory_module.py
last_verified: 2026-09-04
stub: false
---

## 2026-07-28 — R4b：召回记忆整体搬进 contribute_turn_context

（本条为 R4 系列在新 dev 结构上的重放；原始实现 2026-07-25 于 feat/cli-session-capture 分支，该历史不在本分支 mirror 中，条目自含。）

此前 `contribute_instructions` 全文 = 召回记忆列表（逐条含 `(YYYY-MM-DD HH:MM)`
时间戳）→ 每轮必变，是 prod SYSPROMPT-BREAKDOWN 0/17 稳定的两个根因之一。
现拆为：

- `_render_recalled_memories(ctx_data)` — 原渲染逻辑原样提取（文案零改动，
  空列表 → ""）。
- `contribute_instructions` — flag 开 → 恒定常量 `GENERAL_MEMORY_STATIC_INSTRUCTIONS`
...
- `contribute_turn_context` — 返回 `_render_recalled_memories` 输出（搬迁不裁剪，
  "trust the most recent" 指引随行）。

recall hook（gather / limit / token 预算）一行未动。
...
`gather` now recalls over `passive_kinds()` (observation/entity/narrative — distilled knowledge) instead of all kinds, so chat/event/job/bus no longer pollute the per-turn passive injection. They stay explicitly searchable via the `remember` tool. See [[spec]] / [[specs]].

# general_memory_module.py — unified Agent Memory

GeneralMemoryModule — learns world/experience observations each turn (hook_after_event) and injects cross-kind unified memory into context (gather via remember). The single point unified memory feeds the agent loop.
...
UTC) and `contribute_instructions` tells the agent to trust the most recent when two
