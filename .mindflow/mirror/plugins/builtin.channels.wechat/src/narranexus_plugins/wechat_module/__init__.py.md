...
`working_source = WorkingSource.WECHAT`，那是**类定义时**求值的属性访问，开放枚举里
...
# plugins/builtin.channels.wechat/src/narranexus_plugins/wechat_module/__init__.py — package marker

package marker of the WeChat channel plugin. No code belongs here: the host boots plugins from their manifests, and an import-time registration would break the lazy-contribution rule and the distribution excludes.
