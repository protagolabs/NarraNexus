---
code_file: plugins/builtin.channels.wechat/src/narranexus_plugins/wechat_module/wechat_module.py
stub: false
...
`_extract_wechat_reply` 原样保留，现在由 `descriptor.py` 的 `reply_extractor_ref` 按名字指
过来、首次抽取时才解析。
...
行为面的净效果：`wechat_send` 这类回复工具是否被识别，不再取决于本进程有没有 import 过这个模
块，而取决于本发行版是否装了这个渠道。
...
`reply_tool_names = ("wechat_send",)`(NexusPower 投递面声明;门控随 is_bound)。
