---
code_file: plugins/builtin.channels.lark/src/narranexus_plugins/lark_module/__init__.py
...
`working_source = WorkingSource.LARK`，那是**类定义时**求值的属性访问，开放枚举里
...
# plugins/builtin.channels.lark/src/narranexus_plugins/lark_module/__init__.py — package marker

package marker of the Lark channel plugin; import-time side effects are forbidden (lazy contribution rule). No code belongs here: the host boots plugins from their manifests, and an import-time registration would break the lazy-contribution rule and the distribution excludes.
