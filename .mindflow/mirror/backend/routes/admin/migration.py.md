...
本文件仍保留 `from narranexus.platform.settings import settings`（另有
...
**被谁用**：
- `scripts/data_migrations/migrate_users_to_netmind.py`（停服窗口批量模式）：`_amain --execute` 逐条 POST 本接口，或直接调 `identity_migration.execute_migration`（二选一，脚本层决策）。
