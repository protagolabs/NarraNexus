---
code_file: backend/routes/artifacts/_token.py
...
Mirrors `agent_framework/llm/transcription/url_signer.py` (same wire shape so the
...
- This module is a peer-private helper inside `backend/routes/` (leading
  underscore). Import it as `from backend.routes.artifacts import _token`,
  not `from backend.routes.artifacts._token import ...`, to keep call sites
