...
`ROUTES` — this router is the `backend.routes` contribution of `builtin.channels.telegram` (mounted by `backend/plugins_host`, no longer included by `backend/main.py`); its module-internal imports (credential manager / service) are now intra-plugin, and disabling the channel builtin 404s `/api/telegram/*` together with the trigger and MCP tools.
