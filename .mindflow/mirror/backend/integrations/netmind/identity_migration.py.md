...
**依赖谁**：
- `narranexus.platform.utils.db.schema_registry`（`TABLES`）：`classify_identity_columns` 和 `_is_unique_identity_column` 从中自动发现所有身份列，避免列清单硬编码。
- `narranexus.platform.utils.db.db_factory`：操作传入的 `AsyncDatabaseClient`。
