...
- **sweep 降级过、随后来了探测**：两个条件单独都有用例，组合起来一条也没有，
  ...
  钉的是**中间档**（tier 3 → 2），不是归零那一支——归零的会话在**同一趟**
  `prune_idle` 里就被丢掉了（带冷却的条目事件队列恒为空，三条 stale 条件同时
  成立），所以那个状态在公开 API 上根本不存在，第一版是靠直接调私有的
  `_decay_silent_in_memory` 才摆出来的。用不可达状态当范例，会让下一个人建立
  错误的 `prune_idle` ↔ `_half_open` 心智模型，而唯一还活着的那一支反倒没人测。
