...
async def unbind_telegram_bot(
    ...
    ``backend/routes/channels/slack.py:unbind_slack_bot`` for the full rationale.
    """
    auth_err = await _verify_agent_ownership(request, body.agent_id)
    if auth_err:
        return {"success": False, "error": auth_err}
...
# ---- plugin contribution (batch 3c.5): this router belongs to builtin.channels.telegram and is
...
ROUTES = (Contribution("channel_telegram", lambda: RouterSpec(router, "/api/telegram", tags=('Telegram',))),)
