...
| `agent.capabilities.modules` | many | `narranexus.contracts.agent.capability:Capability` | — | `builtin.kernel` | — | L4 capabilities (legacy XYZBaseModule classes through LegacyModuleAdapter); meta carries always_load / channel; every module server is mounted by path on the single MCP host. |
