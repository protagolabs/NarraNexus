# Plugin slots and contract versions

Generated by `scripts/dev/gen_plugin_docs.py` — do not edit by hand.

## Contract versions

| Kind | Version | Stability |
|---|---|---|
| `agent` | 0 | alpha |
| `agent_events` | 0 | alpha |
| `bundle` | 0 | alpha |
| `context_provider` | 0 | alpha |
| `events` | 0 | alpha |
| `framework` | 0 | alpha |
| `hook` | 0 | alpha |
| `llm_client` | 0 | alpha |
| `mcp_server` | 0 | alpha |
| `memory` | 0 | alpha |
| `module` | 0 | alpha |
| `pipeline_profile` | 0 | alpha |
| `provider` | 0 | alpha |
| `route` | 0 | alpha |
| `services` | 0 | alpha |
| `settings` | 0 | alpha |
| `skill` | 0 | alpha |
| `stage_strategy` | 0 | alpha |
| `table` | 0 | alpha |
| `theme` | 0 | alpha |
| `tool` | 0 | alpha |
| `ui` | 0 | alpha |
| `worker` | 0 | alpha |

## Slots

Path → arity → contract → default provider → owner. `distribution-only` slots can be
bound only from the distribution or default layers.

| Path | Arity | Contract | Default | Owner | Flags | Notes |
|---|---|---|---|---|---|---|
| `agent` | one | `narranexus.contracts:Namespace` | `builtin.kernel` | `builtin.kernel` | — | Agent capability domain root. |
| `agent.capabilities` | one | `narranexus.contracts.agent.agent_spec:CapabilitySet` | `builtin.kernel` | `builtin.kernel` | — | Capability namespace; children are the four capability tiers. |
| `agent.capabilities.context_providers` | many | `narranexus.contracts.agent.capability:ContextProvider` | — | `builtin.kernel` | — | Assemble-only capabilities: a stable instruction section and/or a volatile turn-context section. |
| `agent.capabilities.mcp_servers` | many | `narranexus.contracts.mcp_server:McpServerSpec` | — | `builtin.kernel` | — | Site-level MCP servers merged into every agent's tool surface. |
| `agent.capabilities.memory_kinds` | many | `narranexus.contracts.memory:MemoryKindContract` | — | `builtin.kernel` | — | Memory kinds (recall / commit / reflect participants). |
| `agent.capabilities.modules` | many | `narranexus.contracts.agent.capability:Capability` | — | `builtin.kernel` | — | L4 capabilities (legacy XYZBaseModule classes through LegacyModuleAdapter); meta carries mcp_port / always_load / channel. |
| `agent.capabilities.tools` | many | `narranexus.contracts.tool:ToolProvider` | — | `builtin.kernel` | — | Tool providers (Act participants); plugin tools default to tool_search. |
| `backend` | one | `narranexus.contracts:Namespace` | `builtin.kernel` | `builtin.kernel` | — | Backend service domain root. |
| `backend.hooks` | many | `narranexus.kernel.plugins.hooks:HookImplSpec` | — | `builtin.kernel` | — | Hook implementations for declared host hooks (pluggy semantics). |
| `backend.routes` | many | `narranexus.contracts.route:RouterSpec` | — | `builtin.kernel` | — | HTTP routers mounted by the backend host (plugins under /api/x/<id>). |
| `backend.settings` | many | `narranexus.contracts.settings:SettingsSchema` | — | `builtin.kernel` | — | Per-plugin settings schemas (NXP_<ID>_* env > stored row > default). |
| `backend.tables` | many | `narranexus.contracts.table:TableSpec` | — | `builtin.kernel` | — | Database tables (pure data; created by auto_migrate even when the plugin is inactive). |
| `backend.workers` | many | `narranexus.contracts.worker:WorkerSpec` | — | `builtin.kernel` | — | Supervised background workers (workers process or backend lifespan). |
| `content` | one | `narranexus.contracts:Namespace` | `builtin.kernel` | `builtin.kernel` | — | Content-pack domain root. |
| `content.bundles` | many | `narranexus.contracts.bundle:BundleSpec` | — | `builtin.kernel` | — | Team-template .nxbundle files offered by the marketplace. |
| `content.skills` | many | `narranexus.contracts.skill:SkillSpec` | — | `builtin.kernel` | — | Skill directories (SKILL.md) scanned into every agent's skill catalog. |
| `ingress` | one | `narranexus.contracts:Namespace` | `builtin.kernel` | `builtin.kernel` | — | Ingress domain root (channels, triggers). |
| `kernel` | one | `narranexus.kernel:Kernel` | `builtin.kernel` | `builtin.kernel` | distribution-only | Kernel root; never bound directly. |
| `kernel.auth` | one | `narranexus.contracts.services:AuthProvider` | `builtin.auth.local` | `builtin.kernel` | distribution-only | Authentication provider; distribution-level choice. |
| `kernel.db` | one | `narranexus.contracts.services:DatabaseBackend` | `builtin.kernel` | `builtin.kernel` | distribution-only | Database backend (sqlite | sqlite_proxy | mysql). |
| `kernel.events` | one | `narranexus.contracts.services:EventSink` | `builtin.kernel` | `builtin.kernel` | distribution-only | Host event bus implementation. |
| `kernel.secrets` | one | `narranexus.contracts.services:SecretStore` | `builtin.kernel` | `builtin.kernel` | distribution-only | Secret storage (secret_box | keychain | vault). |
| `model` | one | `narranexus.contracts:Namespace` | `builtin.kernel` | `builtin.kernel` | — | Model domain root. |
| `model.clients` | many | `narranexus.contracts.llm_client:LlmClient` | — | `builtin.kernel` | — | Helper-LLM protocol clients (atomic call axis). |
| `model.providers` | many | `narranexus.contracts.provider:ProviderDriver` | — | `builtin.kernel` | — | LLM provider drivers (credential/endpoint axis). |
| `model.resolver` | one | `narranexus.contracts.llm_client:ModelResolver` | `builtin.providers` | `builtin.kernel` | — | Model-name resolution (the three legacy _resolve_model paths, unified in batch 1). |
| `turn` | one | `narranexus.contracts.agent.pipeline:TurnPipeline` | `builtin.turn` | `builtin.kernel` | — | Turn domain root. |
| `turn.pipeline` | one | `narranexus.contracts.agent.pipeline:TurnPipeline` | `builtin.turn` | `builtin.kernel` | — | The whole turn runtime; its provider declares the stage slots. |
| `turn.pipeline.act` | many | `narranexus.contracts.agent.pipeline:ActStrategy` | — | `builtin.kernel` | — | Act stage strategies (agent_loop / direct_trigger / silent); a profile names one. Child of the pipeline so replacing the pipeline owns it. |
| `turn.pipeline.act.framework` | one | `narranexus.contracts.framework:AgentLoopDriver` | `builtin.frameworks.nexus_power` | `builtin.kernel` | — | Agent-loop framework used by the Act stage. |
| `turn.profiles` | many | `narranexus.contracts.agent.pipeline:PipelineProfile` | — | `builtin.kernel` | — | Named pipeline profiles (default/fast/voice/job/silent + plugin-defined). |
| `ui` | one | `narranexus.contracts.ui:Shell` | `builtin.ui` | `builtin.kernel` | distribution-only | Frontend shell; distribution-level choice. |
| `ui.themes` | many | `narranexus.contracts.ui:Theme` | — | `builtin.kernel` | — | Frontend themes (override declared design tokens only). |

## Builtin plugins

| Plugin | Version | Hosts | Provides | Quality |
|---|---|---|---|---|
| `builtin.frameworks.nexus_power` | 1.0.0 | backend | `turn.pipeline.act.framework` | gold |
| `builtin.frameworks.claude_code` | 1.0.0 | backend | `turn.pipeline.act.framework` | gold |
| `builtin.frameworks.codex_cli` | 1.0.0 | backend | `turn.pipeline.act.framework` | gold |
| `builtin.providers` | 1.0.0 | backend, mcp, workers | `model.providers` | gold |
| `builtin.llm_clients` | 1.0.0 | backend, mcp, workers | `model.clients` | gold |
| `builtin.memory_kinds` | 1.0.0 | backend, mcp, workers | `agent.capabilities.memory_kinds` | gold |
| `builtin.nexus_plugins_module` | 1.0.0 | backend, mcp | `agent.capabilities.modules` | gold |
| `builtin.turn` | 1.0.0 | backend | `turn.pipeline`, `turn.pipeline.ingress`, `turn.pipeline.recall`, `turn.pipeline.compose`, `turn.pipeline.assemble`, `turn.pipeline.act`, `turn.pipeline.commit`, `turn.pipeline.reflect`, `turn.profiles` | gold |
| `builtin.awareness` | 1.0.0 | backend, mcp, workers | `agent.capabilities.modules` | gold |
| `builtin.basic_info` | 1.0.0 | backend, mcp, workers | `agent.capabilities.modules` | gold |
| `builtin.chat` | 1.0.0 | backend, mcp, workers | `agent.capabilities.modules` | gold |
| `builtin.social_network` | 1.0.0 | backend, mcp, workers | `agent.capabilities.modules` | gold |
| `builtin.job` | 1.0.0 | backend, mcp, workers | `agent.capabilities.modules` | gold |
| `builtin.skills` | 1.0.0 | backend, mcp, workers | `agent.capabilities.modules` | gold |
| `builtin.message_bus` | 1.0.0 | backend, mcp, workers | `agent.capabilities.modules` | gold |
| `builtin.common_tools` | 1.0.0 | backend, mcp, workers | `agent.capabilities.modules` | gold |
| `builtin.general_memory` | 1.0.0 | backend, mcp, workers | `agent.capabilities.modules` | gold |
| `builtin.home_assistant` | 1.0.0 | backend, mcp, workers | `agent.capabilities.modules` | gold |
| `builtin.channels.lark` | 1.0.0 | backend, mcp, workers | `agent.capabilities.modules` | gold |
| `builtin.channels.slack` | 1.0.0 | backend, mcp, workers | `agent.capabilities.modules` | gold |
| `builtin.channels.telegram` | 1.0.0 | backend, mcp, workers | `agent.capabilities.modules` | gold |
| `builtin.channels.wechat` | 1.0.0 | backend, mcp, workers | `agent.capabilities.modules` | gold |
| `builtin.channels.narramessenger` | 1.0.0 | backend, mcp, workers | `agent.capabilities.modules` | gold |
| `builtin.channels.discord` | 1.0.0 | backend, mcp, workers | `agent.capabilities.modules` | gold |
| `builtin.teams` | 1.0.0 | backend | `backend.routes`, `backend.workers` | gold |
