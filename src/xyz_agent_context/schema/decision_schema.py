...
class ModuleLoadResult(BaseModel):
    ...
    # ===== LLM Decision Error =====
    # Non-None when LLM decision failed and fallback was used.
    # Step 2 surfaces this to the frontend so users know the LLM call had an issue.
    llm_error: Optional[str] = Field(
        default=None,
        description="Error message from LLM decision call (None = success, non-None = fallback used)"
    )
