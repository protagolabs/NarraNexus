...
def _skills_root() -> Optional[Path]:
    ...
    Env override MARKETPLACE_SKILLS_DIR wins; otherwise resolve next to this
    file (marketplace/resources/), so the skills travel with the package in
    every install form. Returns None if it can't be found, so the seed
    degrades to a no-op instead of raising."""
    override = os.environ.get("MARKETPLACE_SKILLS_DIR")
    if override:
        p = Path(override)
        return p if p.is_dir() else None
    candidate = Path(__file__).resolve().parent / "resources" / "marketplace_skills"
