...
`officecli` is **pre-installed** on this platform and already on your `PATH` — do
NOT try to install, download, or `curl | bash` it. Just call it directly.
...
## Live preview in the panel (NarraNexus — IMPORTANT)

The user CANNOT open a raw `.pptx` / `.docx` / `.xlsx` in this chat. Whenever you
create or edit a document, **open a LIVE preview** in the right-side panel so the
user watches it take shape as you build it. Treat this as the delivery step — a
document the user can't see is not delivered.

**Register FIRST, then build.** The live refresh only works while the preview
is open, so you MUST register the document as an artifact **immediately after
`officecli create` — BEFORE you add any slides/content** — and then keep editing.
The user then watches it build slide by slide. If you register only at the end,
the user stares at an empty panel the whole time and the point is lost.

**Name the file meaningfully, and make the title match the filename.** The panel
tab shows the artifact `title`; if that differs from the workspace filename, the
user can't find the file they're looking at. So give the document a real,
descriptive filename up front (e.g. `q4-report.pptx`, not a generic `deck.pptx`)
and pass that **same filename as the `title`**. Keep filenames simple —
lowercase, hyphens/underscores, no spaces or special characters (spaces also
make the officecli path string harder to keep identical).

Exact order:

```
officecli create q4-report.pptx
register_artifact(entry_path="q4-report.pptx", kind="text/html", title="q4-report.pptx")   # title = filename, register NOW (before content)
officecli add q4-report.pptx / --type slide ...      # every edit auto-refreshes the panel
officecli add q4-report.pptx '/slide[1]' --type shape ...
...
```

`kind` is auto-corrected to the office-live kind for .pptx/.docx/.xlsx, so any
value is fine — you don't pick a special kind, and you do NOT run
`officecli watch` or export html/screenshots yourself. You register the doc
**once** (up front); every later `add`/`set`/`remove` auto-refreshes the panel
over SSE — do NOT re-register per edit.

**⚠️ ALWAYS use the SAME relative path for the document — never an absolute
path.** The live refresh works by sharing officecli's in-memory session for the
file, and that session is keyed by the *exact path string* you pass. If you
`create`/`register`/`add` with `deck.pptx` but later edit with
`/abs/path/deck.pptx` (or `./deck.pptx`, or from a different directory), officecli
treats them as different sessions and the preview silently stops updating. So:
run officecli from your working directory and refer to the file as `deck.pptx`
(the same string) in EVERY command — create, register, and all edits.

**Every document you create lives in your working directory.** To find or list
the files you've made, use `ls` in that directory — do NOT `find ~` / `find /`.
A home-wide search is slow and, in the desktop app, walks macOS's protected
folders (Documents, Downloads, Desktop, …), popping a permission dialog for each
one — a terrible experience for the user watching. You already know your files'
names; they're right here in your working directory.

Rules:
- Register up front (right after `create`), not at the end.
- Give the file a real, descriptive filename (simple chars); set `title` to that
  same filename so the panel tab matches the workspace file.
- Use one consistent relative path string for the doc in every officecli command.
- The office document IS the artifact — register the `.pptx/.docx/.xlsx`
  directly (do NOT export to html/png first).
- It renders live and survives refresh / reopen (the backend re-starts the
  preview on demand). This is the preferred delivery path — full fidelity, no
  export needed.
