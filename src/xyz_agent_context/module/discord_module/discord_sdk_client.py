...
class DiscordSDKClient:
    ...
    async def add_reaction(
        ...
        Backs the ``react_to_user_message`` tool. The emoji is URL-encoded
        (Discord expects the raw unicode percent-encoded in the path). Raises
        ``DiscordSDKError`` on failure so the tool can log + swallow
