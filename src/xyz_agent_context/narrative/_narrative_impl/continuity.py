...
# Pattern to detect IM channel template (starts with [<Channel> · ...])
# Matches any IM channel header — the format is generated by ChannelTag.format()
# whenever the tag has a room_id (e.g. [Lark · Alice · ou_xxx · oc_room_id]).
_CHANNEL_TAG_RE = re.compile(r"^\[(?:Lark|Slack|Discord|Telegram)\s*·")

# Pattern to extract sender friendly name from the ChannelTag header line:
# [Lark · 巨灵神 · ou_xxx · oc_room_id]
# Captures the second field (friendly name), which sits between the first and second " · "
_CHANNEL_SENDER_NAME_RE = re.compile(
    r"^\[(?:Lark|Slack|Discord|Telegram)\s*·\s*([^·\]]+?)\s*·"
)

# Pattern to find the last message entry in conversation history:
# [timestamp] @sender:\n    actual message content
_CHANNEL_LAST_MSG_RE = re.compile(
    r"\[[\d]+\]\s*@[^\n]+:\n\s*(.*)",
    re.DOTALL
)
...
def _extract_core_content(text: str) -> str:
    """
    Strip IM channel template wrapper from a query/response,
    keeping only the core message content for continuity detection.

    The channel template looks like:
        [Lark · @sender · ou_xxx · oc_room_id]
    ...
    - ChannelTag.format() in schema/channel_tag.py  → "[Lark · ...]" header line
    ...
    """
    stripped = text.strip()
    if not _CHANNEL_TAG_RE.match(stripped):
        return text

    # Extract sender friendly name from tag line
    sender_name = ""
    name_match = _CHANNEL_SENDER_NAME_RE.match(stripped)
    if name_match:
        sender_name = name_match.group(1).strip()

    # Extract core message body from the last [timestamp] @sender: entry
    msg_match = _CHANNEL_LAST_MSG_RE.search(text)
    if msg_match:
...
"""
        ...
        # Strip channel template wrappers (e.g. Lark headers) so the LLM
