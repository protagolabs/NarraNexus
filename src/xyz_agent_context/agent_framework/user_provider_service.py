...
class UserProviderService:
    ...
    async def set_slot(
        ...
        policy (see cloud_policy). Routes pass the real role; internal
        callers (onboard, OAuth auto-bind) leave the default ``None`` =
        policy already decided upstream. Raises ``CloudPolicyViolation``
        (→ 403 at the route) on a forbidden binding.
        """
        # Validate slot name
