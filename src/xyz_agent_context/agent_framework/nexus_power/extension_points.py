...
_PROTOCOLS = "xyz_agent_context.agent_framework.nexus_power.contracts.protocols"
...
def _stop_default(ctx: SeatContext):
    from xyz_agent_context.agent_framework.nexus_power._nexus_power_impl.harness.stop import NoMoreActionsStop

    return NoMoreActionsStop()


def _compaction_default(ctx: SeatContext):
    from xyz_agent_context.agent_framework.nexus_power._nexus_power_impl.modeling.compaction import ToolResultPruner

    return ToolResultPruner()


def _projector_default(ctx: SeatContext):
    from xyz_agent_context.agent_framework.nexus_power._nexus_power_impl.modeling.projector import PassthroughProjector

    return PassthroughProjector(ctx.base_messages, ctx.tail or (lambda: ""))


def _expression_default(ctx: SeatContext):
    from xyz_agent_context.agent_framework.nexus_power._nexus_power_impl.harness.expression import ExpressionContract

    return ExpressionContract(ctx.options.expressive_tools)


def _disallowed_tools(ctx: SeatContext):
    from xyz_agent_context.agent_framework.nexus_power._nexus_power_impl.tooling.policy import DisallowedToolsLayer

    return DisallowedToolsLayer()


def _workspace_confinement(ctx: SeatContext):
    from xyz_agent_context.agent_framework.nexus_power._nexus_power_impl.tooling.policy import WorkspaceConfinementLayer

    return WorkspaceConfinementLayer()


def _shell_confinement(ctx: SeatContext):
    from xyz_agent_context.agent_framework.nexus_power._nexus_power_impl.tooling.policy import ShellConfinementLayer
