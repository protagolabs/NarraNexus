...
local/desktop mode the driver spawns ``python -m …nexus_loop.runner``
...
``<cwd>/.nexus_loop/`` (constraint C1: the log is recorded, always).
"""
...
async def serve_turn(raw_request: str, write_line: Any) -> int:
    ...
    """
    from xyz_agent_context.agent_framework.nexus_loop.assembly import (
        TurnRequest,
        run_turn_events,
    )
    from xyz_agent_context.agent_framework.nexus_loop.contracts.options import TurnOptions
    from xyz_agent_context.agent_framework.nexus_loop._nexus_loop_impl.event_adapter import (
        LegacyEventAdapter,
    )
    ...
    log = FileEventLogWriter(
        request.thread_id,
        os.path.join(options.cwd, ".nexus_loop", f"{request.thread_id}.ndjson"),
    )
...
def main() -> None:
    """``python -m xyz_agent_context.agent_framework.nexus_loop.runner``:
