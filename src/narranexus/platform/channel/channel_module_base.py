...
class ChannelModuleBase(XYZBaseModule):
    ...
    async def cleanup_for_agent(self, agent_id: str, db) -> dict[str, int]:
        ...
        Default implementation handles the common case:
          1. Look up the agent's binding for this channel in the generic
             ``channel_credentials`` store.
    ...
    async def expressive_tools(self, ctx_data: Any = None) -> list[str]:
        ...
        from narranexus.platform.schema.hook_schema import is_plain_text_turn
