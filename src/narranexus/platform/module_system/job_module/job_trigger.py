...
Usage:
    # Standalone
    uv run python -m narranexus.platform.module_system.job_module.job_trigger

    # With custom interval
    uv run python -m narranexus.platform.module_system.job_module.job_trigger --interval 30
"""
...
from narranexus.platform.module_system.job_module._job_context_builder import build_execution_prompt
...
Examples:
  ...
  uv run python -m narranexus.platform.module_system.job_module.job_trigger --interval 30 --workers 3

  # Run once (for testing)
  uv run python -m narranexus.platform.module_system.job_module.job_trigger --once
"""
    )
...
async def test_execute_single_job():
    ...
    Usage:
        uv run python -c "import asyncio; from narranexus.platform.module_system.job_module.job_trigger import test_execute_single_job; asyncio.run(test_execute_single_job())"
    """
    import narranexus.platform.settings  # noqa: F401 - Ensure .env is loaded
