...
=====
    # all workers (default)
    python -m narranexus.platform.module_system.run_worker_supervisor

    # a subset — lets cloud split workers across containers with no code change
    python -m narranexus.platform.module_system.run_worker_supervisor --only poller,jobs
    python -m narranexus.platform.module_system.run_worker_supervisor --exclude channels

    # split channels themselves (orthogonal to --only), same as the old
    # run_channel_triggers --only:
    python -m narranexus.platform.module_system.run_worker_supervisor --only channels --channels lark,slack
...
from narranexus.platform.module_system.run_channel_triggers import start_channel_triggers
...
def main(argv: "list[str] | None" = None) -> int:
    """CLI entry — also what the one-release ``xyz_agent_context`` path shim calls."""
