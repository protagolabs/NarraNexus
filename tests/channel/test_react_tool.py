...
high-freedom, agent-driven half of the IM early-feedback design. Each IM
module exposes the same-named tool with a shared semantic emoji vocabulary
(on_it/done/thumbs_up/heart/problem) mapped to its platform's own tokens,
backed by the per-channel SDK ``add_reaction``. WeChat has no reaction API and
returns the unsupported envelope. Every path is best-effort — a failing SDK
call returns ``{"success": false}`` and never raises.
