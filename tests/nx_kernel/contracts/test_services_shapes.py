...
@author: Bin Liang
@date: 2026-09-07
@description: The kernel.* service contracts match the implementations they were traced from (no silent drift) — the ABC and every concrete backend.
"""
...
def _assert_same_shape(cls, label):
    ...
    defaults; an implementation may accept MORE (e.g. SQLiteBackend.execute_write's private
    retry knob) only with a default, so a contract-shaped call always binds."""
