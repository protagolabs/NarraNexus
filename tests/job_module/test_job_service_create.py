...
@pytest.mark.asyncio
async def test_create_jobs_batch_reuses_existing_instance_in_dependency_map(db_client):
    service = JobInstanceService(db_client)
    existing = await service.create_job_with_instance(
        agent_id="agent_1",
        user_id="user_1",
    ...
    assert result["success"], result
    assert result["key_to_id"]["prerequisite"] == existing["instance_id"]
    dependent = await InstanceRepository(db_client).get_by_id(
        result["key_to_id"]["dependent"]
    )
    assert dependent is not None
    assert dependent.dependencies == [existing["instance_id"]]
