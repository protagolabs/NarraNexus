...
async def test_tool_call_stamps_the_run_agent_stage(db_client):
    ...
    run-agent phase's derived label (``step.3.4_Run Agent``), not the old
    ``step.3_agent_loop`` that disagreed with the progress-derived stage and
    made ``events.current_stage`` flap between two strings for one phase."""
