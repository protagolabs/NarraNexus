...
An external subject (ext:…) keeps its own scope — so each external IM conversation
is its own tenant, and DERIVED work (a job an external user creates) stays external
automatically (its stored user_id is the ext: subject). Any other user_id collapses
...
def test_external_subject_keeps_its_scope():
    # External IM turn — and any job/callback carrying the same ext: subject.
    assert _resolve_scope_user_id("ext:slack:room1", "owner_x") == "ext:slack:room1"


def test_external_subject_kept_even_without_creator():
    assert _resolve_scope_user_id("ext:slack:room1", None) == "ext:slack:room1"


def test_external_job_propagation_is_automatic():
    # A job created by an external user stores user_id=ext:... ; job_trigger later
    # runs it with that value → scope stays external WITHOUT any flag.
    job_user_id = "ext:narramessenger:abc123def456"
