...
import backend.integrations.netmind.netmind_provisioner as prov
...
async def test_capture_stamps_all_netmind_rows(db_client, monkeypatch):
    ...
    monkeypatch.setattr(
        "backend.integrations.netmind.netmind_auth_client.NetmindAuthClient.verify_token",
        fake_verify,
    )
...
async def test_existing_user_backfilled_on_login(db_client, monkeypatch):
    ...
    monkeypatch.setattr(
        "backend.integrations.netmind.netmind_key_client.NetmindKeyClient.create_key",
        _boom_mint,
    )
