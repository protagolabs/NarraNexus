...
def test_funnel_endpoint_fires_setup_events(funnel_client, captured_events):
    for event in ("setup_entered", "setup_skipped", "setup_completed"):
        ...
        evt = next(e for e in captured_events if e["event"] == event)
        assert evt["user_id"] == "ivy"


def test_funnel_endpoint_rejects_unknown_event(funnel_client, captured_events):
...
def test_funnel_endpoint_requires_auth(funnel_client, captured_events):
...
def test_funnel_endpoint_ignores_client_properties(funnel_client, captured_events):
    # Client-supplied properties must never reach persistence — a client could
    ...
    assert r.status_code == 200
    evt = next(e for e in captured_events if e["event"] == "setup_entered")
