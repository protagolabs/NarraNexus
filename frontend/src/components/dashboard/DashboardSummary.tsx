...
const CHIP_ORDER: ChipDef[] = [
  { health: 'error',           labelKey: 'error',    dotCls: 'bg-[var(--color-red-500)]',     textCls: 'text-[var(--color-red-500)]' },
  { health: 'acknowledged',    labelKey: 'ack',      dotCls: 'bg-slate-500',                  textCls: 'text-slate-600 dark:text-slate-400' },
  { health: 'warning',         labelKey: 'blocked',  dotCls: 'bg-[var(--color-yellow-500)]',  textCls: 'text-[var(--color-yellow-500)]' },
  { health: 'paused',          labelKey: 'paused',   dotCls: 'bg-[var(--color-yellow-500)]',  textCls: 'text-[var(--color-yellow-500)]' },
  { health: 'healthy_running', labelKey: 'running',  dotCls: 'bg-[var(--color-green-500)]',   textCls: 'text-[var(--color-green-500)]' },
  { health: 'healthy_idle',    labelKey: 'idle',     dotCls: 'bg-[var(--color-silicon)]',     textCls: 'text-[var(--text-primary)]' },
  { health: 'idle_long',       labelKey: 'quiet',    dotCls: 'bg-gray-400',                   textCls: 'text-gray-500' },
];
    ...
    >
      ...
      <div className="flex items-baseline gap-1.5 tabular-nums">
        ...
        <span className="text-[10px] uppercase tracking-[0.1em] text-[var(--text-tertiary)] font-[family-name:var(--font-mono)]">
          {total === 1 ? t('dashboard.summary.agent') : t('dashboard.summary.agents')}
        </span>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs tabular-nums">
        {CHIP_ORDER.map((c) => {
          ...
          return (
            <span
              key={c.health}
              title={t(`dashboard.summary.tooltip.${c.health}`)}
            ...
            >
              ...
              <span className={`text-[11px] ${c.textCls}`}>{t(`dashboard.summary.chip.${c.labelKey}`)}</span>
            </span>
          );
        })}
      </div>
