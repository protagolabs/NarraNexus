...
export function DashboardSkeleton() {
  return (
    <div data-testid="dashboard-skeleton" className="p-6 space-y-4 animate-pulse">
      ...
      <div className="h-10 w-full rounded-[var(--radius-lg)] bg-[var(--bg-elevated)]" />
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {Array.from({ length: 4 }).map((_, i) => (
          <div
            key={i}
            className="h-28 rounded-[var(--radius-2xl)] border border-[var(--border-default)] bg-[var(--bg-elevated)]"
          />
        ))}
      </div>
    </div>
