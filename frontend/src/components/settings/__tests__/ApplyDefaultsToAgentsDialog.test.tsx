...
test('agent slot checkable, helper slot disabled when zero overrides', () => {
  render(<ApplyDefaultsToAgentsDialog isOpen stats={STATS} onClose={() => {}} onApply={vi.fn()} />);
...
test('apply calls onApply with only checked slots', async () => {
  ...
  render(<ApplyDefaultsToAgentsDialog isOpen stats={STATS} onClose={() => {}} onApply={onApply} />);
...
test('save-only button closes without applying', () => {
  const onClose = vi.fn();
  const onApply = vi.fn();
  render(<ApplyDefaultsToAgentsDialog isOpen stats={STATS} onClose={onClose} onApply={onApply} />);
