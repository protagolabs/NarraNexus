...
export function ApplyDefaultsToAgentsDialog({ isOpen, stats, onClose, onApply }: Props) {
              ...
              <label key={key} className="flex items-center gap-2 text-[13px]">
                ...
                <span className="text-[var(--nm-ink50)]">
                  {disabled
                    ...
                    : t('pages.settings.modelDefaults.willClear', '{{n}} agents will be cleared').replace('{{n}}', String(count))}
                </span>
              </label>
            );
          })}
