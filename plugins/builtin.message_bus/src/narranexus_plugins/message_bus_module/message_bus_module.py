...
from narranexus.platform.module_system.base import (
    XYZBaseModule,
    mcp_server_url,
...
#: label, body indentation and cut marker included). Unread direct messages
...
class MessageBusModule(XYZBaseModule):
    ...
    def _volatile_context_parts(self, ctx_data: ContextData) -> list:
        ...
        if unread:
            ...
            # order, newest last), always keeping the newest one.
            kept = len(rows)
            used = 0
            ...
            if omitted:
                ...
                how = (
                    "read them with " + ", ".join(calls)
                    if calls
                    else "their senders have no read_history handle"
                )
                if calls and any(not c for _, c in omitted):
                    how += " (some come from senders with no read_history handle)"
                parts.append(
                    f"- {len(omitted)} older unread message(s) not shown: this "
                    f"list's size budget is spent; {how}."
                )
    ...
    async def gather(self, ctx_data: ContextData) -> ContextData:
        ...
        try:
            ...
            try:
                ...
                if db:
                    from narranexus.platform.message_bus.agent_discovery_sync import (
...
async def _get_default_bus_async():
    ...
    try:
        from narranexus.platform.message_bus import LocalMessageBus
        from narranexus.platform.utils.db.db_factory import get_db_client
