...
# succeed (verified: scripts/spikes/spike_codex_approval_probe.py). We close that
        ...
        with tempfile.TemporaryDirectory(
            ...
            try:
                async for notification in stream:
                    ...
                    dump = (
                        getattr(notification, "model_dump")(mode="json", by_alias=False)
                    ...
                    if "method" not in dump and hasattr(notification, "method"):
                        dump["method"] = getattr(notification, "method")
                    if "payload" not in dump and hasattr(notification, "payload"):
                        # If payload is a pydantic model, expand it.
                        pl = getattr(notification, "payload")
                        dump["payload"] = (
                            getattr(pl, "model_dump")(mode="json", by_alias=False)
