...
@author: Bin Liang
@date: 2026-09-04
@description: Package contract of `builtin.home_assistant`: the manifest file equals the host's builtin manifest, the facade resolves the module class. Import isolation between builtin packages (no builtin may import another builtin's narranexus_plugins.* module) is enforced separately by the "builtin packages are independent (api facades excepted)" import-linter contract in pyproject.toml, not by this test.
"""
...
def test_manifest_and_facade():
    ...
    assert on_disk == next(d for d in BUILTIN_MANIFEST_DATA if d["id"] == "builtin.home_assistant")
