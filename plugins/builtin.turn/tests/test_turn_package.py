...
@author: Bin Liang
@date: 2026-09-04
@description: Package contract of `builtin.turn`: the manifest file equals the host's builtin manifest and every provided symbol resolves inside this package.
"""
...
def test_manifest_and_provides():
    ...
    data = next(d for d in BUILTIN_MANIFEST_DATA if d["id"] == "builtin.turn")
    assert on_disk == data
