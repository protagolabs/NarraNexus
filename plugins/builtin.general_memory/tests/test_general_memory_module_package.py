...
def test_manifest_and_facade():
    from narranexus_plugins.general_memory_module import api

    on_disk = json.loads((ROOT / "narranexus-plugin.json").read_text())
    cls = api.module_class()
    assert cls.__module__.startswith("narranexus_plugins.general_memory_module") and cls.get_config().name == cls.__name__
